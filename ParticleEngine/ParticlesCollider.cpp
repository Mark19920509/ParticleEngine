...
Sphere *ParticlesCollider::GetInsideSpheres()
{
    if (m_InsideSphere.size() == 0)
    {
        return &m_NullSphere;
    }
    return &(*m_InsideSphere.begin());
}
...
int ParticlesCollider::GetInsideSpheresCount() const
{
    return m_InsideSphere.size();
}
