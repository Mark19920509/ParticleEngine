...
void getGaussianBlurKernel1D( int s, float o, float* kernel, size_t kernelsize )
{
	...
	for ( int i = 0 ; i < s ; ++i )
	{
		...
		const float f = 1.f/(2.506628274631000502415765284811f*o*o) * expf( -d*d/(2.f*o*o) );
		kernel[i] = f;
		sum += f;
	}
...
void getGaussianBlurKernel2D( int s, float o, float* kernel, size_t kernelsize )
{
	...
	for ( int j = 0 ; j < s ; ++j )
	{
		...
		for ( int i = 0 ; i < s ; ++i )
		{
			...
			const float f = 1.f/(6.283185307179586476925286766559f*o*o) * expf( -(dx*dx + dy*dy)/(2.f*o*o) );
			kernel[i+j*s] = f;
			sum += f;
		}
	}
