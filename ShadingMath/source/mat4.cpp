...
mat4 fromToRotation( const vec3& from, const vec3& to )
{
	...
	{
		vec3 u, vTemp; /* temporary storage vectors */
		...
		vTemp[0] = x[0] - to[0];   vTemp[1] = x[1] - to[1];   vTemp[2] = x[2] - to[2];

		c1 = 2.0f / dot(u, u);
		c2 = 2.0f / dot(vTemp, vTemp);
		c3 = c1 * c2  * dot(u, vTemp);

		for (i = 0; i < 3; i++) 
		{
			for (j = 0; j < 3; j++) 
			{
				mtx[j][i] =  - c1 * u[i] * u[j] - c2 * vTemp[i] * vTemp[j] + c3 * vTemp[i] * u[j];
			}
			mtx[i][i] += 1.0f;
		}
	}
