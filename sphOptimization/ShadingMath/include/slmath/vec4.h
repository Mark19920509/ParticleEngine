...
public:
	 ...
	 */
	vec4( float x0, float y0, float z0, float w0 = 0);
	 ...
	 * @param w0 The vector W-component (3)
	 */
	vec4( const slmath::vec3& v0, float w0 = 0);
    ...
    /** Returns true if vector this is superior or equal. */
	bool		operator>=( const slmath::vec4& o ) const;

    /** Returns true if vector this is inferior or equal. */
	bool		operator<=( const slmath::vec4& o ) const;
...
/** 
 * Returns the vector normalized to unit (1) length and the vector lentgh.
 * @ingroup vec_util
 */
