    ...
    bool isAPIerror)
{
    ...
    else
    {
        if(isAPIerror)
        {
            DEBUG_OUT("Error: "<< message.c_str() << " Error code : ");
            DEBUG_OUT(getOpenCLErrorCodeStr(input) << std::endl);
        }
        else
            error(message);   
        return SDK_FAILURE;
...
SDKCommon::buildOpenCLProgram(cl_program &program, const cl_context& context, const buildProgramData &buildData)
{
    ...
    if(buildData.binaryName.size() != 0)
    {
        ...
        {
            DEBUG_OUT("Failed to load binary kernel file : " << kernelPath.c_str() << std::endl);
            return SDK_FAILURE;
        }
    ...
    {
        DEBUG_OUT(kernelPath.c_str());
        kernelPath.append(buildData.kernelName.c_str());
        if(!kernelFile.open(kernelPath.c_str()))//bool
        {
            DEBUG_OUT("Failed to load kernel file: " << kernelPath.c_str() << std::endl);
            return SDK_FAILURE;
        }
        const char * source = kernelFile.source().c_str();
        size_t sourceSize[] = {strlen(source)};
    ...
    if(buildData.flagsFileName.size() != 0)
    {
        ...
        if(!flagsFile.open(flagsPath.c_str()))
        {
            DEBUG_OUT( "Failed to load flags file: " << flagsPath.c_str() << std::endl);
            return SDK_FAILURE;
        }
        flagsFile.replaceNewlineWithSpaces();
        const char * flags = flagsFile.source().c_str();
    ...
    if(flagsStr.size() != 0)
        DEBUG_OUT("Build Options are : " << flagsStr.c_str() << std::endl);
...
{
    DEBUG_OUT("Error: "<<errorMsg<<std::endl);
}
...
{
    DEBUG_OUT("Expected Error: "<<errorMsg<<std::endl);
}
