...
void RainDemo::Initialize()
{
    ...
    for (int i = 0; i < length; i++)
    {
        for (int j = 0; j < length; j++)
        {
            ...
            x = 2.0f * (1.0f - std::powf(2.73,(s * 6 * pi) / (6 * pi))) * std::cosf(s * 6 * pi) * std::cosf((t * 2 * pi) / 2) * std::cosf((t * 2 * pi) / 2);
            y = 2.0f * (-1.0f + std::powf(2.73,(s * 6 * pi) / (6 * pi))) * std::sinf(s * 6 * pi) * std::cosf((t * 2 * pi) / 2) * std::cosf((t * 2 * pi) / 2);
            z = 1.0f - std::powf(2.73,(s * 6 *pi) / (3 * pi)) - std::sinf(t * 2 * pi) + std::powf(2.73,(s * 6 * pi) / (6 * pi)) * std::sinf(t * 2 *pi);
...
{
    renderer.InitializeParticles(GetParticlesCount(), 9, 0);
}
