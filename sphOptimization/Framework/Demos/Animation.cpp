...
void Animation::Initialize()
{
    ...
    {
        m_StartPositions            [i].w = Compress(slmath::vec4(1.0f, 0.0f, 0.0f, 1.0f));
        m_ParametricSphere          [i].w = Compress(slmath::vec4(0.0f, 0.0f, 1.0f, 1.0f));
        m_ParametricCone            [i].w = Compress(slmath::vec4(0.0f, 1.0f, 1.0f, 1.0f));
        m_ParametricTorus           [i].w = Compress(slmath::vec4(0.0f, 0.0f, 1.0f, 1.0f));
        m_ParametricCylinder        [i].w = Compress(slmath::vec4(1.0f, 0.0f, 1.0f, 1.0f));
        m_ParametricTear            [i].w = Compress(slmath::vec4(0.0f, 1.0f, 0.0f, 1.0f));
        m_ParametricCrossCap        [i].w = Compress(slmath::vec4(1.0f, 0.0f, 1.0f, 1.0f));
        m_ParametricSteinersRoman   [i].w = Compress(slmath::vec4(0.0f, 1.0f, 0.0f, 1.0f));
        m_ParametricSeaShell        [i].w = Compress(slmath::vec4(0.0f, 1.0f, 0.0f, 1.0f));
        m_ParametricDini            [i].w = Compress(slmath::vec4(1.0f, 0.0f, 0.0f, 1.0f));
        m_ParametricHeart           [i].w = Compress(slmath::vec4(0.0f, 0.0f, 1.0f, 1.0f));
    }
...
void Animation::Simulate()
{
 ...
 //   m_Emiter.Update();
    ...
    if (transition % 2 == 0)
    {
        m_Transformer.Update(0.005f, delay);
    }
    else
    {
        m_Transformer.Update(0.01f, -delay);
    }

    float currentStep = m_Transformer.GetCurrentStep();

    if (transition % 2 == 0 && currentStep >= 1.0f)
    {
        m_Transformer.Initialize(m_Shapes[m_CurrentShape], m_StartPositions, -delay);
    ...
    else if (transition % 2 == 1 && currentStep >= 1.0f)
    {
        m_Transformer.SetCurrentStep(1.0f - slmath::saturate(currentStep));
        int nextStep = m_CurrentShape < m_ShapesCount - 1 ?  m_CurrentShape + 1 : 0;   
        m_Transformer.Initialize(m_StartPositions, m_Shapes[nextStep]);
        m_CurrentShape++;
